...
const EditProfile = () => {
    ...
    const deactive_link = (link) => {
        apiFech.updateProject(`https://determined-cyan-vest.cyclic.app/sociallinks/${link._id}`, { headers: { 'content-type': 'application/json', }, body: JSON.stringify({ update: "deactivate" }) }, (res => res.modifiedCount === 1 ? alert("Deactivate successfully") : alert("Deactivate unsuccessfully")))
    }
    const del_link = (link) => {
        const confirmDel = window.confirm("If you want to delete")
        confirmDel && apiFech.rmProject(`https://determined-cyan-vest.cyclic.app/sociallinks/${link._id}`, {}, (res => res.deletedCount === 1 ? alert("Delete successfully") : alert("Delete unsuccessfully")))
    }
    ...
    const update_link = (link) => {
        ...
        if (updatedLink.name && updatedLink.url) {
            ...
            apiFech.updateProject(`https://determined-cyan-vest.cyclic.app/sociallinks/${link._id}`, { headers: { 'content-type': 'application/json', }, body: JSON.stringify(updatedLink) }, (res => res.modifiedCount === 1 ? alert("Update successfully") : alert("Update unsuccessfully")))
        }
    }
    const send_cv_url = () => {
        if (cv_url.new_cv) {
            apiFech.updateProject(`https://determined-cyan-vest.cyclic.app/sociallinks/${cvId}`, { headers: { 'content-type': 'application/json', }, body: JSON.stringify(cv_url) }, (res => res.modifiedCount === 1 ? alert("Cv update successfully") : alert("cv update unsuccessfully")), '', () => setSending(true))
        }
    }
    sending && setTimeout(() => {
        setSending(false)
