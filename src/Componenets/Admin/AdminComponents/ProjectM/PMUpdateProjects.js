...
const ModalEdit = ({ modalOpen, handleClose, projects, removeProject }) => {
    ...
    const handleSubmit = (e) => {
        ...
        apiFech.updateProject(`https://determined-cyan-vest.cyclic.app/projects/${projects._id}`, { body: formData }, (res) => console.log(res))
    }
    const style = {
        position: 'absolute',
        top: '50%',
...
const PMUpdateProjects = ({ projects, response, }) => {
    ...
    const removeProject = () => {
        apiFech.rmProject('https://determined-cyan-vest.cyclic.app/projects/' + projects._id, null, res => {
            if (res.deletedCount) {
                response('delete')
                handleClose()
            }
